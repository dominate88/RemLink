...
func SetSecureHeader(w http.ResponseWriter) {
	...
	w.Header().Set("Server", "RemLinkOpenSource")
	...
	w.Header().Set("Content-Security-Policy", "default-src 'self' data: blob:; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; frame-ancestors 'self'; base-uri 'self'; block-all-mixed-content")
