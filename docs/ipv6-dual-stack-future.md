...
> **状态更新（2026-07-25）**：V2 全部完成，已实现于 `ipv6-v1` 分支，含 1.1（v6 精细 ACL）/ 1.2（v6 FakeDNS AAAA + v6 DNAT）/ 1.3（v6 审计）/ 1.4（v6 DNS 拦截）/ 2.1（组级独立 v6 出网隔离 + 前端配置）。以下各节保留"原排期与依赖"供回溯；3.1（服务端 v6 接入监听）仍属 v3 前瞻，本轮未做。
...
| 项 | 业务驱动力 | 依赖 | 工作量 | 建议档位 | 状态 |
|---|---|---|---|---|---|
| 1.1 v6 ACL | 内网 v6 资源要精细管控 | 前端校验+v1§1.3+共享v6解析helper | 中（原估低偏乐观） | v2 首选 | ✅ V2 完成 |
| 1.3 v6 审计 | 合规要 v6 流量可见 | 与 1.1 共享 helper | 低–中（存储侧极低） | v2 | ✅ V2 完成 |
| 1.2 v6 FakeDNS | 开了 FakeDNS 的组要真双栈 | 1.4 同批 | 中 | v2 | ✅ V2 完成 |
| 1.4 v6 DNS 拦截 | 域名策略要覆盖 v6 | 1.2 | 低 | v2（随 1.2） | ✅ V2 完成 |
| 2.1 组级 v6 隔离 | 多租户 v6 出网隔离 | 无 | 中 | v2（独立立项） | ✅ V2 完成 |
| 3.1 v6 接入监听 | 纯 v6 客户端要接入 | DTLS 栈 | 大 | v3 前瞻 | ⬜ 未做（v3） |
