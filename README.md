...
2. **`ipv6_cidr` 须用你有权路由的 GUA 段**：
   - 家庭 / 企业宽带通常从运营商拿到 **IPv6-PD**（如 `2409:8a62:856:3010::/60`）。PD 内含 16 个 `/64`，**取其中一个未被 LAN 占用的 `/64`** 作为池，例如 `2409:8a62:856:3011::/64`（避开路由器已用在 LAN 的 `3010::/64`）。
   - **不要把整个 `/60` 当池**——会与 LAN 段重叠，导致 NDP / 出网混乱。
...
- `fd00::/8`（ULA，站点本地地址）**公网不可路由**。用 ULA 池（如 `fd00:1::/64`）只能内部互通 / 互 ping、验证数据面，**出不了公网**——这是 RFC 4193 的预期行为，不是 bug。
- 要访问公网 v6，必须用上文的 **GUA 段**。

#### 配置与验证示例

假设运营商 IPv6-PD 为 `2409:8a62:856:3010::/60`：

1. 后台「虚拟网络」：
   - `ipv6_cidr` = `2409:8a62:856:3011::/64`
   - `global_nat` 保持默认 `true`（NAT66）
2. 保存后**重启 RemLink** 服务，客户端重连。
3. 客户端验证（Windows 示例）：
   ```powershell
   # 通网关（证明隧道头 + 接口 v6 + 服务器侧 v6 端点 OK）
   ping 2409:8a62:856:3011::1
   # 通公网（证明 NAT66 / 路由 OK）
...
| 网关通、公网不通（纯路由） | 上游未把 `ipv6_cidr` 段回指到服务器 |
| 用 ULA 池公网不通 | 预期行为（ULA 不可公网路由），换 GUA 段 |
