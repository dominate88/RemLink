  ...
  <div>
    <el-form :model="searchForm" ref="searchForm" :inline="true" class="search-form">
      ...
      <el-form-item label="关键字:">
        <el-input size="mini" v-model="searchForm.keyword" clearable placeholder="操作目标/详情" style="width: 180px" @keydown.enter.native="searchEnterFun"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table ref="multipleTable"
      ...
      :header-cell-style="{backgroundColor:'var(--bg-header)'}"
      border>

      <el-table-column prop="id" label="ID" sortable="custom" width="80"></el-table-column>
      <el-table-column prop="admin_user" label="管理员" width="120"></el-table-column>
      <el-table-column prop="op_type" label="操作类型" width="110">
        <template slot-scope="{ row }">
          <el-tag size="small" type="primary" disable-transitions>{{ row.op_type }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="op_target" label="操作目标" min-width="150">
        <template slot-scope="{ row }">
          <span v-if="row.op_target">{{ row.op_target }}</span>
          <span v-else style="color:var(--text-placeholder)">-</span>
        </template>
      </el-table-column>
      <el-table-column prop="detail" label="详情" min-width="180"></el-table-column>
      <el-table-column prop="client_ip" label="操作IP" width="130"></el-table-column>
      <el-table-column prop="created_at" label="操作时间" width="155" :formatter="tableDateFormat"></el-table-column>
    </el-table>
    <div class="sh-20"></div>
    <el-pagination
      background
...
.search-form >>> .el-form-item__label { font-size: 12px; }
